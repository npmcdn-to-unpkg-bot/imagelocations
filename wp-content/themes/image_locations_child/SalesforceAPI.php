...
//                CURLOPT_SSL_VERIFYHOST => false,
  ...
  public function login($username, $password, $security_token) {
    ...
    $curl = curl_init($this->base_url . '/services/oauth2/token');
    ...
    $this->new_instance_url = $login->instance_url;
  ...
  protected function request($path, $params = [], $method = self::METH_GET, $headers = []) {
    ...
    echo "</pre>";
    echo "<br />";
    echo "URL: " . $this->instance_url . $path;
    echo "<br />";
...
//                curl_setopt($this->handle, CURLOPT_CUSTOMREQUEST, null);
  ...
  public function create_account($request_params) {
    $url = $this->new_instance_url . "/services/data/v32.0/sobjects/Account/";
  ...
  public function create_contact($request_params) {
    $url = $this->new_instance_url . "/services/data/v20.0/sobjects/contact";
    $content = json_encode($request_params);
  ...
  public function create_location($request_params) {
    $url = $this->new_instance_url . "/services/data/v20.0/sobjects/Location__c";
    $content = json_encode($request_params);
